  ...
  for (const address of addresses) {
    try {
      ...
      for (const match of html.matchAll(/<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi)) {
        ...
        found.push(normalize({ company: parts[0] || '招聘单位', title, city: '全国', salary: '薪资面议', degree: '不限', date: today,
...
const items = [...new Map(all.map(x => [`${x.source}|${x.company}|${x.title}|${x.url}`, x])).values()]
